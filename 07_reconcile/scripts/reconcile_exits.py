...
Usage:
    python 07_reconcile/scripts/reconcile_exits.py                # all open positions w/ SELL in flight
    python 07_reconcile/scripts/reconcile_exits.py --id 5
    python 07_reconcile/scripts/reconcile_exits.py --limit 10
    python 07_reconcile/scripts/reconcile_exits.py --dry-run
    python 07_reconcile/scripts/reconcile_exits.py --verbose
"""
...
# Cross-layer imports — process_approved for get_connection,
# reconcile_orders for Alpaca client + status helpers.
sys.path.insert(0, str(PROJECT_ROOT / "04_approval" / "scripts"))
import process_approved as pa  # noqa: E402

sys.path.insert(0, str(PROJECT_ROOT / "07_reconcile" / "scripts"))
import reconcile_orders as rc  # reuse _alpaca_client + status partitions  # noqa: E402

# Import shared alert_telegram for exit-fill push notifications
