...
Usage:
    python scripts/generate_signals.py                # all active symbols
    python scripts/generate_signals.py --all
    python scripts/generate_signals.py --symbol NVDA
    python scripts/generate_signals.py --dry-run       # stdout only
    python scripts/generate_signals.py --threshold 60  # custom alert threshold
"""
...
# ---------------------------------------------------------------------------
# Env helpers — support both /app/ (Docker) and project-root (local-dev)
# ---------------------------------------------------------------------------

def _load_env(filename: str) -> None:
    """Populate os.environ from a dotenv-style file."""
    for prefix in ["/app", str(PROJECT_ROOT)]:
...
_load_env(".env.db")
_load_env(".env.alpaca")
_load_env(".env.telegram")
